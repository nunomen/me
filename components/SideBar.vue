<template>
    <nav class="sticky top-0 left-0 z-20 h-screen flex flex-col bg-[#1E1E2F] text-white" :class="{'max-w-[80px]': isCollapsed, 'w-[18rem]': !isCollapsed}">
      <div class="flex justify-between items-center px-4 py-2">
        ...
        <button @click="toggleCollapse" class="border border-purple-700 bg-[#2C2C3C] w-8 h-8 rounded-md flex justify-center items-center">
      ...
      <div v-for="(section, index) in sections" :key="index" class="pt-4 px-4">
        ...
        <ul class="space-y-1">
          <li v-for="item in section.items" :key="item.href" class="text-gray-300">
            <a :href="item.href" :title="item.title" @click.prevent="setActiveLink(item.href)" class="flex items-center gap-3 py-2 rounded-md hover:bg-purple-700 hover:text-white">
